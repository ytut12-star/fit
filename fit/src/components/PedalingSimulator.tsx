...
import { t, type Lang } from '../translations'; // 💡 번역 임포트
...
interface SimParams {
  stackMm: number; reachMm: number; seatTubeAngleDeg: number; saddleHeightMm: number; brpSetbackMm: number; crankLengthMm: number; inseamCm: number; calfLengthCm: number | null; footSizeMm: number; cleatOffsetMm: number; pedalStackCorrectionMm: number; upperBodyCm: number; ridingStyle: 'performance' | 'comfort' | 'endurance'; armLengthCm: number; stemLengthMm: number; spacerHeightMm: number; handlebarReachMm: number; drivetrainHoodReachMm: number; stemAngleDeg: number;
}

const HEAD_TUBE_ANGLE_DEG = 73; const EXPECTED_TORSO_RATIO = 0.52; const EXPECTED_ARM_RATIO = 0.34;
const ANKLE_HEIGHT_MM = 70; const HEEL_TO_ANKLE_RATIO = 0.2; const CLEAT_FROM_HEEL_RATIO = 0.62; const PEDAL_AXLE_BASE_RADIUS_MM = 12;
const PEDAL_STACK_BASE_MM = 15; const SHOE_CLEAT_STACK_MM = 20;
const TORSO_LENGTH_RATIO = 0.52; const UPPER_ARM_RATIO = 0.47; const FOREARM_RATIO = 0.53;
const TUBE_THICKNESS = { headtube: 45, downtube: 55, toptube: 35, seattube: 32, chainstay: 25, seatpost: 27.2, cockpit: 40 };
const CADENCE_PERIOD_MS = 2000;

function estimateArmCm(height: number, upperBody: number, armInputMode: FittingInput['armInputMode'], armLengthInput: number | null, wingspan: number | null): number {
...
function buildSimParams(result: FittingResult, input: FittingInput): SimParams | null {
  ...
  return {
    stackMm: result.stack, reachMm: result.reach, seatTubeAngleDeg: result.matchedFrame.seatTubeAngle ?? 73.5,
    saddleHeightMm: result.saddleHeight, brpSetbackMm: result.brpSetback, crankLengthMm: result.crankLength,
    inseamCm: input.inseam, calfLengthCm: input.calfLength, footSizeMm: result.footSize ?? 260, cleatOffsetMm: result.cleatOffset,
    pedalStackCorrectionMm: result.pedalStackCorrection, upperBodyCm: result.upperBody, ridingStyle: input.ridingStyle,
    armLengthCm, spacerHeightMm: result.spacerHeight, stemLengthMm: result.stemLength, handlebarReachMm: result.handlebarReach,
    drivetrainHoodReachMm: result.drivetrainHoodReach, stemAngleDeg: input.stemAngle ?? -6,
  };
}

function drawFittingRider(ctx: CanvasRenderingContext2D, w: number, h: number, params: SimParams, crankAngle: number, lang: Lang) {
  ...
  const cx = w * 0.38; const cy = h * 0.82;
  ...
  const saddleX = px(saddleDx); const saddleY = py(saddleDy);
  const headTopX = px(params.reachMm); const headTopY = py(params.stackMm); const headBottomY = headTopY + 30 * scale;
  ...
  const seatClusterX = px(seatClusterDx); const seatClusterY = py(seatClusterDy);
  const brpX = px(-params.brpSetbackMm); const brpY = saddleY;
  ...
  ctx.fillStyle = '#27272a'; ctx.beginPath(); ctx.arc(cx, cy, 6, 0, Math.PI * 2); ctx.fill();
  ...
  ctx.strokeStyle = '#52525b'; ctx.lineWidth = tw(TUBE_THICKNESS.seattube);
  ...
  ctx.beginPath(); ctx.moveTo(cx, cy); ctx.lineTo(px(-410), cy); ctx.stroke();
  ...
  ctx.fillStyle = '#18181b'; ctx.beginPath(); ctx.ellipse(saddleX, saddleY, saddleLength / 2, 7.5 * scale, 0, 0, Math.PI * 2); ctx.fill();
  ctx.strokeStyle = '#71717a'; ctx.lineWidth = 1; ctx.stroke();

  ctx.fillStyle = '#f43f5e'; ctx.beginPath(); ctx.arc(brpX, brpY, 3, 0, Math.PI * 2); ctx.fill();
  ...
  ctx.beginPath(); ctx.moveTo(brpX, cy); ctx.lineTo(cx, cy); ctx.stroke(); ctx.setLineDash([]);

  const hipX = brpX + 35 * scale; const hipY = brpY - 70 * scale;
  ...
  if (params.calfLengthCm && params.calfLengthCm > 0) {
    ...
    shinMm = maxLegExtension * shinRatio; thighMm = maxLegExtension * (1 - shinRatio);
  } else {
    thighMm = maxLegExtension * 0.52; shinMm = maxLegExtension * 0.48;
  }
  const pedalRadiusPx = Math.max(6, PEDAL_AXLE_BASE_RADIUS_MM + params.pedalStackCorrectionMm) * scale;
  const totalStackMm = PEDAL_STACK_BASE_MM + SHOE_CLEAT_STACK_MM + params.pedalStackCorrectionMm;
  const soleLocalY = -totalStackMm * scale;
  const cleatFromHeelMm = params.footSizeMm * CLEAT_FROM_HEEL_RATIO + params.cleatOffsetMm;
  const heelLocalX = -cleatFromHeelMm * scale; const toeLocalX = heelLocalX + params.footSizeMm * scale;
  ...
  const thighLen = thighMm * scale; const shinLen = shinMm * scale;
  const maxLegReach = (thighLen + shinLen) * 0.999;

  let kneeX = 0, kneeY = 0, kneeAngleRad = Math.PI;
  for (let pass = 0; pass < 2; pass++) {
    const legDx = ankleX - hipX; const legDy = ankleY - hipY;
  ...
  ctx.save(); ctx.translate(pedalX, pedalY); ctx.rotate(footAngle);
  ctx.fillStyle = '#27272a'; ctx.strokeStyle = '#71717a'; ctx.lineWidth = 1.5;
  ctx.beginPath(); ctx.moveTo(heelLocalX, soleLocalY); ctx.lineTo(toeLocalX, soleLocalY); ctx.lineTo(ankleLocalX, ankleLocalY);
  ctx.closePath(); ctx.fill(); ctx.stroke();
  ctx.fillStyle = '#10b981'; ctx.beginPath(); ctx.arc(0, soleLocalY, 3, 0, Math.PI * 2); ctx.fill(); ctx.restore();

  ctx.lineCap = 'round'; ctx.lineJoin = 'round';
  ctx.beginPath(); ctx.moveTo(hipX, hipY); ctx.lineTo(kneeX, kneeY);
  ctx.strokeStyle = 'rgba(34, 211, 238, 0.5)'; ctx.lineWidth = tw(150); ctx.stroke();
  ctx.strokeStyle = '#22d3ee'; ctx.lineWidth = 2; ctx.stroke();
  ctx.beginPath(); ctx.moveTo(kneeX, kneeY); ctx.lineTo(ankleX, ankleY);
  ctx.strokeStyle = 'rgba(8, 145, 178, 0.5)'; ctx.lineWidth = tw(110); ctx.stroke();
  ctx.strokeStyle = '#0891b2'; ctx.lineWidth = 2; ctx.stroke();
  ctx.lineCap = 'butt';

  const joints: [number, number, string][] = [[hipX, hipY, '#f43f5e'], [kneeX, kneeY, '#22d3ee'], [ankleX, ankleY, '#0891b2']];
  for (const [jx, jy, color] of joints) {
    ctx.fillStyle = color; ctx.beginPath(); ctx.arc(jx, jy, 4.5, 0, Math.PI * 2); ctx.fill();
    ctx.lineWidth = 1.5; ctx.strokeStyle = '#18181b'; ctx.stroke();
  }
  ...
  const stemStartX = px(stemStartMmX); const stemStartY = py(stemStartMmY);
  const cockpitAngleRad = ((90 - HEAD_TUBE_ANGLE_DEG + params.stemAngleDeg) * Math.PI) / 180;
  const totalCockpitLenMm = params.stemLengthMm + params.handlebarReachMm + params.drivetrainHoodReachMm + 10;
  ...
  if (params.ridingStyle === 'performance') { armBendRatio = 0.85; } else if (params.ridingStyle === 'comfort') { armBendRatio = 0.92; }
  const armLen = params.armLengthCm * 10 * scale;
  const effectiveArmTargetLen = armLen * armBendRatio;
  const targetDx = handX - hipX; const targetDy = handY - hipY;
  ...
  const shoulderX = hipX + Math.cos(finalTorsoAngle) * torsoLen; const shoulderY = hipY + Math.sin(finalTorsoAngle) * torsoLen;
  const upperArmLen = armLen * UPPER_ARM_RATIO; const forearmLen = armLen * FOREARM_RATIO;
  const armDx = handX - shoulderX; const armDy = handY - shoulderY;
  ...
  ctx.lineCap = 'round'; ctx.lineJoin = 'round';
  ctx.beginPath(); ctx.moveTo(hipX, hipY); ctx.lineTo(shoulderX, shoulderY);
  ctx.strokeStyle = 'rgba(167, 139, 250, 0.4)'; ctx.lineWidth = tw(180); ctx.stroke();
  ctx.strokeStyle = '#a78bfa'; ctx.lineWidth = 2; ctx.stroke();
  ctx.beginPath(); ctx.moveTo(shoulderX, shoulderY); ctx.lineTo(elbowX, elbowY);
  ctx.strokeStyle = 'rgba(196, 181, 253, 0.4)'; ctx.lineWidth = tw(100); ctx.stroke();
  ctx.strokeStyle = '#c4b5fd'; ctx.lineWidth = 2; ctx.stroke();
  ctx.beginPath(); ctx.moveTo(elbowX, elbowY); ctx.lineTo(handX, handY);
  ctx.strokeStyle = 'rgba(221, 214, 254, 0.4)'; ctx.lineWidth = tw(90); ctx.stroke();
  ctx.strokeStyle = '#ddd6fe'; ctx.lineWidth = 2; ctx.stroke();
  ctx.lineCap = 'butt';

  const upperJoints: [number, number, string][] = [[shoulderX, shoulderY, '#a78bfa'], [elbowX, elbowY, '#c4b5fd'], [handX, handY, '#f43f5e']];
  ...
  // 💡 캔버스 내부 텍스트 번역 적용
  ctx.fillText(t[lang].simCanvasBB, cx, cy + 20);
  ctx.fillText(t[lang].simCanvasBRP, brpX, brpY - 12);
  ctx.fillText(t[lang].simCanvasLever, handX, handY - 12);
  ctx.fillText(t[lang].simCanvasKnee(kneeAngleDeg.toFixed(0)), kneeX + 26, kneeY);

  ctx.fillStyle = '#71717a'; ctx.font = '10px sans-serif'; ctx.textAlign = 'left';
  ctx.fillText(t[lang].simCanvasInfo(params.stackMm, params.reachMm), 12, 24);
}

export function PedalingSimulator({ result, input, lang }: PedalingSimulatorProps) {
  ...
  useEffect(() => {
    ...
    const animate = (now: number) => {
      ...
      if (paramsRef.current) {
        drawFittingRider(ctx, rect.width, rect.height, paramsRef.current, angle, lang); // 💡 lang 전달
      }
    ...
    <div className="rounded-2xl border border-zinc-800 bg-zinc-900/60 p-4">
      <div className="mb-3 flex items-center justify-between">
        <div className="flex items-center gap-2">
          ...
          <div>
            <h3 className="text-sm font-bold text-zinc-100">{t[lang].simTitle}</h3>
            <p className="text-[11px] text-zinc-500">{t[lang].simDesc}</p>
          </div>
        </div>
      ...
      <div className="mt-3 flex flex-wrap items-center justify-between gap-2 border-t border-zinc-800/60 pt-2.5 text-[11px] text-zinc-400">
        <div className="flex items-center gap-3">
          <span className="flex items-center gap-1.5">
            ...
            <span className="text-zinc-300">{t[lang].simLegend1}</span>
            <span className="text-[10px] text-zinc-500">{t[lang].simLegend1Sub}</span>
          </span>
          <span className="flex items-center gap-1.5">
            <span className="h-2 w-2 rounded-full bg-cyan-400 ring-2 ring-cyan-400/20" />
            <span className="text-zinc-300">{t[lang].simLegend2}</span>
          </span>
          <span className="flex items-center gap-1.5">
            <span className="h-2 w-2 rounded-full bg-violet-400 ring-2 ring-violet-400/20" />
            <span className="text-zinc-300">{t[lang].simLegend3}</span>
          </span>
        </div>
